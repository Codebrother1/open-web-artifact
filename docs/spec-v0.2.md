...
These number rules apply to finite floating-point annotations as well as integers. Arbitrary-precision parsing or a host language's default JSON encoder is not necessarily equivalent. At every object depth, a raw JSON text containing two member names that decode to the same string MUST be rejected as `OWA_INVALID_JSON_VALUE` before manifest validation or artifact identity is assigned. Escaped and literal spellings of the same name count as duplicates; case differences and distinct Unicode sequences do not. An overflowing numeric token is invalid even if a later member would replace its value. This rule applies to raw JSON inputs; an already-parsed object contains no evidence of discarded members and cannot be checked retroactively. Ordinary JSON syntax failures remain `OWA_INVALID_JSON`.
...
- Duplicate decoded JSON member names are rejected at raw parsing boundaries as specified under canonical manifest encoding. Unpaired-surrogate fixtures pin preservation behavior, but implementations must disclose parser limitations rather than substitute characters.
