...
- Both materialize the portable anchor [`pack-cross-language-anchor`](v0.2/pack.json) as the regular-file baseline and check it against its **unchanged static expectations**; then they create a real FIFO (`mkfifo` / `syscall.Mkfifo`) and a real bound Unix-domain socket (`net.createServer().listen` / `net.Listen("unix")`) at the root and inside nested directories — including names with known extensions such as `queue.txt` and `notes.html` — pack again, and assert that the manifest, canonical JSON, artifact digest, ordered file entries, sorted blob digest set and every blob's bytes are identical and still equal the anchor, that the skipped names are absent, and that the socket received **no** connection. A tree whose `index.html` is only a FIFO or a socket fails `OWA_MISSING_ENTRYPOINT`; a tree of only special entries fails `OWA_INVALID_MANIFEST`; symlinks to a regular file, a directory, a missing target, an external file, a FIFO and a socket all fail `OWA_SYMLINK`.
- "Never opened" is proven for the writer-less FIFO (which blocks any read-open forever) in a **separate process under an external deadline**: the JavaScript test spawns a child Node process and kills it with `SIGKILL` after 30 s, awaiting `close` so the child is reaped; the Go test re-executes the test binary under `exec.CommandContext` with the same deadline (`WaitDelay` closes the pipes). A self-check in each suite confirms a child that really opens the FIFO blocks, is killed and reaped within a 2 s deadline, so a timeout can never leave a blocked process or handle behind.
