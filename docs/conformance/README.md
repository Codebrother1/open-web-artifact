...
- **Duplicate-content OCI import:** import indexes layers by digest, collapsing repeated-content layer entries that carry distinct path annotations. Such manifests remain valid; this suite does not introduce new OCI path/digest semantics. The OCI property uses unique content, while manifest/pack cases still exercise deduplication.
